...
<?php endif; ?>
    ...
    <div id="addcomment" class="col-md-12 col-sm-12 col-xs-12 no-padding-lr">
        ...
        <div class="blog-comment-form">
            ...
            'author' => '<input id="author" name="author" type="text" class="big-input alt-font comment-field" placeholder="'.esc_html__( '* Your Name','brando').'" value="' .esc_attr( $commenter['comment_author'] ) . '" size="30" />' ,
            'email'  => '<input id="email" name="email" class="big-input alt-font comment-field" type="text" placeholder="'.esc_html__( '* Your Email','brando').'" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" required/>' ,
            'cookies' => '<div class="comment-form-cookies-consent"><input id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' />' .
 			'<label for="wp-comment-cookies-consent">' . esc_html__( 'Save my name, email, and website in this browser for the next time I comment.', 'brando' ) . '</label></div><span class="required">'.esc_html__( '*Please complete all fields correctly', 'brando' ).'</span>',
             )),
            'comment_field' => '<textarea id="comment" class="big-input alt-font comment-field" name="comment" placeholder="'.esc_html__( '* Your Message','brando').'" aria-required="true"></textarea>',
            ...
            'id_form'           => 'commentform',
            'id_submit'         => 'submit',
            'class_submit'      => 'highlight-button-dark btn btn-medium comment-button alt-font',
            'title_reply'       => '',
            'title_reply_to'    => esc_html__( 'Leave a Reply to %s', 'brando' ),
