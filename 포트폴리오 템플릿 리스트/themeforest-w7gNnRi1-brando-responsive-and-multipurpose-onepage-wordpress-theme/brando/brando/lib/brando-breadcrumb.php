...
if(!class_exists('brando_breadcrumb_navigation_xt')){
	class brando_breadcrumb_navigation_xt {
		...
		public function __construct() {
			$this->opt = array(
				...
				// Text displayed for the search page.
					'title_search' => esc_html__('Search','brando'),
				...
				// The prefix that is used for mouseover link (e.g.: "Browse to: Archive")
					'urltitle_prefix' => esc_html__('Browse to: ','brando'),
				...
				// Prefix for tags (Simple Tagging Plugin)
					'tag_page_prefix' => esc_html__('Tag: ','brando'),
				// Prefix for tags (Simple Tagging Plugin)
					'tag_page_suffix' => '', 
				// Text displayed for a 404 error page, , only being used if 'use404' => true
					'title_404' => esc_html__('404','brando'),
				// For post type
					'title_portfolio' => esc_html__('Portfolio','brando'),
				// Display current item as link?
					'link_current_item' => false,
				// URL title of current item, only being used if 'link_current_item' => true
					'current_item_urltitle' => esc_html__('Link of current page (click to refresh)','brando'),
		...
		function display() {
			...
			if ( ($this->opt['static_frontpage'] === true) AND ($this->opt['home_display'] === true)) {		// Hide HOME if it is disabled in the options
				if ($this->opt['home_link'] === true) {			// Link home or just display text
					$bcn_homelink = '<a href="' . esc_url($this->opt['url_home']) . '" title="' . $this->opt['urltitle_prefix'] . $this->opt['title_home'] . $this->opt['urltitle_suffix'] . '">' . esc_attr($this->opt['title_home']) . '</a>';
				} else {
			...
			/* -------- BLOG LINK -------- */
			$bcn_bloglink = '<li><a href="' . esc_url(home_url('/') . $this->opt['url_blog']) . '" title="' . $this->opt['urltitle_prefix'] . $this->opt['title_blog'] . $this->opt['urltitle_suffix'] . '">' . esc_attr($this->opt['title_blog']) . '</a></li>';
			...
			if ($this->opt['link_current_item']) {
				$curitem_urlprefix = '<a title="' . esc_attr($this->opt['current_item_urltitle']) . '" href="' . esc_url(home_url( $wp->request )) . '">';
				$curitem_urlsuffix = '</a>';
			}
			...
			elseif ( is_singular('portfolio'))				$swg_type = 'singleportfolio';
			...
			************************************************* */
			...
			case 'page':
				...
				if( strtolower($bcn_pagetitle) != strtolower($this->opt['title_home']) ) {  // Check if we are not on home
					...
					$brando_breadcrumb_hide_title = brando_option('brando_breadcrumb_hide_title');
					$result_array['last']['prefix'] = $this->opt['page_prefix'];
					if ( $brando_breadcrumb_hide_title == 1 ) {
			...
			case 'search':
				...
				$post_type_array = array('portfolio','attachment');
				// Add category
				if($this->opt['singleblogpost_category_display'] === true && !in_array( get_post_type(), $post_type_array ) ) {
					$category = get_the_category(); 
					$category_link = get_category_link( $category[0]->term_id );
					
					$result_array['middle'] .= $this->opt['separator'] . $this->opt['singleblogpost_category_prefix'] . '<li><a href="'.esc_url( $category_link ).'" title="'.esc_attr($category[0]->cat_name).'">'.esc_attr($category[0]->cat_name).'</a></li>' . esc_attr($this->opt['singleblogpost_category_suffix']);
				}
				
				if(get_post_type() == 'portfolio'){
					$result_array['middle'] .= '<li>'.esc_html__('Portfolio','brando').'</li>';
					
					if( $this->opt['singleblogpost_category_display'] === true ) {
						$category = get_the_terms( $post->Id, 'portfolio-category' );
						if( $category ){
							$category_link = get_term_link($category[0]->term_id, 'portfolio-category');						
							
							$result_array['middle'] .= $this->opt['separator'] . $this->opt['singleblogpost_category_prefix'] . '<li><a href="'.esc_url( $category_link ).'" title="'.esc_attr($category[0]->name).'">'.esc_attr($category[0]->name).'</a></li>' . esc_attr($this->opt['singleblogpost_category_suffix']);
						}
					}
				}
				...
				$brando_breadcrumb_hide_title_portfolio = brando_option('brando_breadcrumb_hide_title_portfolio'); 
				if ( $brando_breadcrumb_hide_title_portfolio == 1 ) {
					$result_array['last']['title'] = '<li>'.esc_attr($bcn_post_title).'</li>';
				}
			...
			case 'singlepost':
				...
				if ( ($this->opt['posttitle_maxlen'] >= 1) and ( strlen($bcn_post_title) > $this->opt['posttitle_maxlen']) )  
					$bcn_post_title = substr($bcn_post_title, 0, $this->opt['posttitle_maxlen']-1) . '...';
				$brando_breadcrumb_hide_title_post = brando_option('brando_breadcrumb_hide_title_post'); 
				if ( $brando_breadcrumb_hide_title_post == 1 ) {
					$result_array['last']['title'] = '<li>'.esc_attr($bcn_post_title).'</li>';
				}
				$result_array['last']['suffix'] = $this->opt['singleblogpost_suffix'];
			...
			case 'singlepostcategory':
				...
				$bcn_post_title =  '<li><a href="'.get_permalink( get_page_by_path( ('blog'))).'">'.esc_attr($bcn_post_title).'</a></li>';
	        ...
	        case 'categories':
				...
				$cat_name = '<li>'.esc_attr($object->cat_name).'</li>';
				// Get parents of current category
				$parent_id  = $object->category_parent;
				$cat_breadcrumbs = '';
				while ($parent_id) {
					$category   = get_category($parent_id);
					$cat_breadcrumbs = '<li><a href="' . esc_url(get_category_link($category->cat_ID)) . '" title="' . $this->opt['urltitle_prefix'] . $category->cat_name . $this->opt['urltitle_suffix'] . '">' . esc_attr($category->cat_name) . '</a></li>' . $this->opt['separator'] . $cat_breadcrumbs;
			...
			case 'portfolio':
				...
				$object = $wp_query->get_queried_object();
				$cat_name = '<li>'.esc_attr($object->name).'</li>';
				// Get parents of current category
				$parent_id  = isset( $object->parent ) ? $object->parent : '';
				$cat_breadcrumbs = '';
				while ($parent_id) {
					$category   = get_category($parent_id);
					$cat_breadcrumbs = '<li><a href="' . esc_url(get_category_link($category->cat_ID)) . '" title="' . $this->opt['urltitle_prefix'] . $category->cat_name . $this->opt['urltitle_suffix'] . '">' . esc_attr($category->cat_name) . '</a></li>' . $this->opt['separator'] . $cat_breadcrumbs;
			...
			case 'else':
				...
				////////////////////////////////////////////////////////////////////
				$default = '<li>'.esc_attr($this->opt['title_blog']).'</li>';
	...
	} // END class brando_breadcrumb_navigation_xt
}