    ...
    if ( ! class_exists( 'reduxCorePanel' ) ) {
        ...
        class reduxCorePanel {
            ...
            private function panel_template() {
                ...
                // Do we support JS?
                echo '<noscript><div class="no-js">' . __( 'Warning- This options panel will not work properly without javascript!', 'brando' ) . '</div></noscript>';
...
//                    }
            ...
            function notification_bar() {
                ...
                if ( isset( $this->parent->transients['last_save_mode'] ) ) {
                    ...
                    if ( $this->parent->transients['last_save_mode'] == "import" ) {
                         ...
                         */
                        echo '<div class="admin-notice notice-blue saved_notice"><strong>' . apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", __( 'Settings Imported!', 'brando' ) ) . '</strong></div>';
                        //exit();
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-text-{$this->parent->args['opt_name']}", __( 'All Defaults Restored!', 'brando' ) ) . '</strong></div>';
                         ...
                         */
                        echo '<div class="saved_notice admin-notice notice-green">' . apply_filters( "redux-saved-text-{$this->parent->args['opt_name']}", '<strong>'.__( 'Settings Saved!', 'brando' ) ).'</strong>' . '</div>';
                    }
                 ...
                 */
                echo '<div class="redux-save-warn notice-yellow"><strong>' . apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", __( 'Settings have changed, you should save them!', 'brando' ) ) . '</strong></div>';
                    ...
                    if ( $developer_theme_file ) {
                        ...
                        if ( $core_version && $developer_version && version_compare( $developer_version, $core_version, '<' ) ) {
                            ...
                            <div id="message" class="error redux-message">
                                <p><?php _e( '<strong>Your panel has bundled outdated copies of Redux Framework template files</strong> &#8211; if you encounter functionality issues this could be the reason. Ensure you update or remove them.', 'brando' ); ?></p>
                            </div>
                            <?php
                            return;
                        }
            ...
            function output_section( $k ) {
                $this->do_settings_sections( $this->parent->args['opt_name'] . $k . '_section_group' );
            }
            ...
            function do_settings_fields($page, $section) {
                ...
                foreach ( (array) $wp_settings_fields[$page][$section] as $field ) {
                    ...
                    if(isset($field['args']['type']) && $field['args']['type'] == 'accordion' && $field['args']['position'] == 'start'){
                        echo '</table>';
                        echo '<div class="brando-admin-tabs">';
                            echo '<div class="brando-admin-title">';
                                echo '<div class="brando-admin-tab-title">'.$field['args']['title'];
                                    echo '<span class="brando-admin-subtitle">'.$field['args']['subtitle'].'</span>';
                                echo '</div>';
                                echo '<span class="brando-icon el el-icon-plus"></span>';
                            echo '</div>';
                        echo '<table class="form-table brando-admin-tab-slidingdiv">';
                    }

                    $array_type = array('accordion');
                    if (!in_array($field['args']['type'], $array_type)) {
                        echo "<tr{$class}>";
                    ...
                    if(isset($field['args']['type']) && $field['args']['type'] == 'accordion' && $field['args']['position'] == 'end'){
                        echo '</table>';
                            echo '</div>';
                                echo '<table class="form-table">';
                    }
