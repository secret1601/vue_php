...
$brando_all_demo_setup = array(
					array(
						...
						'theme_name' 		=> esc_html__('Application', 'brando'),
						'theme_preview_url' => 'https://brando.themezaa.com/application/',
						'theme_screenshot' 	=> get_template_directory_uri() . '/assets/images/home-application.jpg',
					),
					array(
						'theme_id' 			=> 'corporate',
						'theme_name' 		=> esc_html__('Corporate', 'brando'),
						'theme_preview_url' => 'https://brando.themezaa.com/corporate/',
						'theme_screenshot' 	=> get_template_directory_uri() . '/assets/images/home-classic-corporate.jpg',
					),
					array(
						'theme_id' 			=> 'agency',
						'theme_name' 		=> esc_html__('Agency', 'brando'),
						'theme_preview_url' => 'https://brando.themezaa.com/agency/',
						'theme_screenshot' 	=> get_template_directory_uri() . '/assets/images/agency.jpg',
					),
					array(
						'theme_id' 			=> 'travel',
						'theme_name' 		=> esc_html__('Travel', 'brando'),
						'theme_preview_url' => 'https://brando.themezaa.com/travel/',
						'theme_screenshot' 	=> get_template_directory_uri() . '/assets/images/travel.jpg',
					),
					array(
						'theme_id' 			=> 'restaurant',
						'theme_name' 		=> esc_html__('Restaurant', 'brando'),
						'theme_preview_url' => 'https://brando.themezaa.com/restaurant/',
...
if( brando_is_theme_licence_active() ) {
	$brando_import_export_content .= '<div class="brando-theme-browser">';
	foreach ( $brando_all_demo_setup as $key => $demo_setup ) {
		...
		$brando_import_export_content .= '<div class="theme">
						                <div class="theme-wrapper">
						                    ...
						                    if( class_exists( 'Brando_Addons' ) && !class_exists('WP_Import') ) {
	...
	$brando_import_export_content .= '<div class="clear"></div>
	</div>';
} else {
    $brando_import_export_content .= '<div class="import-content demo-licence-not-activated">
        <div class="licence-not-activated"><i class="fas fa-info-circle"></i><span>'.__( 'Please activate your Brando theme license copy to enjoy premium feature of demo data import.', 'brando' ).'</span></div><br>
        <a class="brando-demo-import-licence" href="'.admin_url( 'themes.php?page=brando-licence-activation' ).'">'.__( 'Activate Brando WordPress Theme License', 'brando' ).'</a>
    </div>';
}

$brando_import_export_notice = '';
if( !class_exists( 'Brando_Addons' ) ) {
	$brando_import_export_notice .= '<strong class="font-16px" style="color:red">'.esc_html__( 'Brando Addons plugin is required to use theme features as well as import demo data. So please make sure to install Brando Addons plugin here: Appearance > Install Plugins or activate it under Plugins menu if it is installed already and then come back to this page to import demo data.', 'brando').'</strong><br/><br/>';
}
if( class_exists( 'Brando_Addons' ) && class_exists( 'WP_Importer' ) ){
			...
			'content'		=> '
				...
				<strong>'.esc_html__('Import Demo Content', 'brando').'</strong>
		 		...
		 		<ul class="import-export-desc">
		 			<li><i class="fas fa-check"></i>'.esc_html__('Memory Limit of 256 MB and max execution time (php time limit) of 180 seconds or more.', 'brando').'</li>
		 			<li><i class="fas fa-check"></i>'.esc_html__('Brando Addon must be activated for Custom post type and Shortcodes to import.', 'brando').'</li>
		 			<li><i class="fas fa-check"></i>'.esc_html__('Visual Composer must be activated for content to import.', 'brando').'</li>
		 			<li><i class="fas fa-check"></i>'.esc_html__('Contact Form 7 must be activated for form data to import.', 'brando').'</li>
		 			<li><i class="fas fa-check"></i>'.esc_html__('Newsletter Manager must be activated for newsletter form data to import.', 'brando').'</li>
		 		</ul>
				' . $brando_import_export_content . '
