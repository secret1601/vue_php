...
$brando_enable_title = (isset($brando_options['brando_category_enable_title_wrapper'])) ? $brando_options['brando_category_enable_title_wrapper'] : '';
$brando_title_parallax_effect = (isset($brando_options['brando_category_title_parallax_effect'])) ? $brando_options['brando_category_title_parallax_effect'] : '';
...
if( $brando_enable_title == 1 ){
    ...
    echo '<section '.$brando_post_class.'>';
        ...
        $brando_desc = term_description( $brando_t_id, 'portfolio-category' );

        $brando_page_title_image = (isset($brando_options['brando_category_title_background'])) ? $brando_options['brando_category_title_background'] : '';
        $brando_image = ( $brando_term_meta['image_url'] ) ? $brando_term_meta['image_url'] : $brando_page_title_image['url'];
        
        if( $brando_image ){
            echo '<img class="parallax-background-img" src="'.esc_url($brando_image).'" alt="background-img" />';
        }        
                    ...
                    echo '<div class="col-text-middle-main">';
                        echo '<div class="col-text-middle">';
                            echo '<div class="col-md-12 col-sm-12 text-center">';
                                if( $brando_title ){
                                    echo '<h1 class="alt-font white-text font-weight-600 xs-title-extra-large no-margin entry-title brando-title-text">'.esc_attr($brando_title).'</h1>';
                                }
                                if( $brando_desc ){
                                    echo '<div class="alt-font title-small xs-text-large white-text text-uppercase margin-one-top display-block brando-subtitle-text">'.$brando_desc.'</div>';
                                }
                            echo '</div>';
                        echo '</div>';
                    echo '</div>';
